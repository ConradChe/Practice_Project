 ...
 * @description 更新学生function
 * @author XuTianYi
 * @creation time 2017年12月22日 下午4:45:12
 */

public class UpdateStudentFunction implements IFunction{

	@Override
	public DaoResult doFunction(Map params, Map context) throws Exception {
		
		/*----------获取所需参数------------*/
		String studentId = params.get("student_id").toString(); //学生编号
		...
		// String studentPhoto = StringUtil.convertObject(params.get("student_photo").toString()); //学生照片
		String majorId = params.get("major_id").toString(); //专业ID
		String majorName = params.get("major_name").toString(); //专业名称
		...
		param.put("operate_id", operaterId);
		param.put("operate_name", operaterName);
		paramList.add(param);
		
		//添加关联表信息
		Map param2 = new HashMap();
		param2.put("funcId", "hex_student_updateStudentAndMajor");
		param2.put("studetn_id", studentId);
